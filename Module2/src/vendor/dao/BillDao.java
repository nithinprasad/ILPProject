...
public class BillDao {
	...
	public ArrayList<PurchaseOrder> searchPO(String poNumber,String vendorId) throws SQLException {
		// TODO Auto-generated method stub
		DBConnection dbConnection = new DBConnection();
		Connection connection = dbConnection.getConnection();
		PreparedStatement preparedStatement=connection.prepareStatement("SELECT * FROM PO WHERE PO_NO IN(SELECT PO_NO FROM PO_VENDOR WHERE V_NO=?) AND STATUS=? AND PO_NO LIKE ?");
		preparedStatement.setString(1,vendorId);
		preparedStatement.setString(2,"APPROVED");
	...
	public ArrayList<Appointment> searchAppoinment(String billNo,
			String vendorId) throws SQLException {
		// TODO Auto-generated method stub
		DBConnection dbConnection = new DBConnection();
		Connection connection = dbConnection.getConnection();
		logger.debug("fetching appointment details of particular vendor"+vendorId);
		PreparedStatement preparedStatement = connection
				.prepareStatement("SELECT * FROM APPOINTMENT WHERE V_NO=? AND BILL_NO IN(SELECT BILL_NO FROM VENDOR_BILL WHERE READY_FOR_SHIPMENT='NO')AND BILL_NO LIKE ? ORDER BY STATUS ,BILL_NO ASC");
		preparedStatement.setString(1, vendorId);
	...
	public VendorBill getVendorBill(String billNo) throws SQLException {
		// TODO Auto-generated method stub
		DBConnection dbConnection = new DBConnection();
		Connection connection = dbConnection.getConnection();
		logger.debug("fetching Vendor Bill details of Bill No"+billNo);
		PreparedStatement preparedStatement=connection.prepareStatement("SELECT * FROM VENDOR_BILL WHERE BILL_NO=?");
		preparedStatement.setString(1,billNo);
		ResultSet resultSet=preparedStatement.executeQuery();
		if(resultSet.next())
		{
			...
			String poNo=resultSet.getString(1);
			String vendorNo=resultSet.getString(2);
			String typeOfDelivery=resultSet.getString(4);
			String status=resultSet.getString(5);
			String readyForShipment=resultSet.getString(6);
			String directToStore=resultSet.getString(7);
			Date dateOFDelivery=resultSet.getDate(8);
			String orderBy=resultSet.getString(9);
			VendorBill vendorBill=new VendorBill(poNo, vendorNo, billNo, typeOfDelivery, status, readyForShipment, directToStore, dateOFDelivery, orderBy);
			return vendorBill;
		}
		return null;
	}

	public Shipment getShipment(String billNo) throws SQLException {
		// TODO Auto-generated method stub
		DBConnection dbConnection = new DBConnection();
		Connection connection = dbConnection.getConnection();
		logger.debug("fetching Shipment details of Bill No"+billNo);
		PreparedStatement preparedStatement=connection.prepareStatement("SELECT * FROM SHIPMENT WHERE BILL_NO=?");
		preparedStatement.setString(1,billNo);
		ResultSet resultSet=preparedStatement.executeQuery();
		if(resultSet.next())
		{
			String poNumber=resultSet.getString(2);
			String truckNo=resultSet.getString(3);
			Shipment shipment=new Shipment(billNo, poNumber, truckNo);
			return shipment;
		}
		return null;
	}
	...
	public BillDetails getBillDetails(String billNo) throws SQLException {
		...
		DBConnection dbConnection = new DBConnection();
		Connection connection = dbConnection.getConnection();
		logger.debug("fetching Bill details of Bill No"+billNo);
		PreparedStatement preparedStatement=connection.prepareStatement("SELECT * FROM BILL_DETAILS WHERE BILL_NO=?");
		preparedStatement.setString(1,billNo);
		ResultSet resultSet=preparedStatement.executeQuery();
		if(resultSet.next())
		{
