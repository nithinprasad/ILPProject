...
public class VendorProductDao {
	...
	public String deleteVendorProductOnProductId(String vendorId, String productId)
			throws SQLException {
		/**
		...
		PreparedStatement preparedStatement = connection
				.prepareStatement("DELETE FROM VENDOR_PRODUCT WHERE PRODUCT_ID NOT IN(SELECT PRODUCT_ID FROM PO_PRODUCT WHERE PO_NO IN(SELECT PO_NO FROM PO_VENDOR WHERE V_NO=?))AND PRODUCT_ID=?");
	int result=0;
	...
	public ArrayList<VendorProduct> searchProduct(String prodId, String vendorId) throws SQLException{
		...
		PreparedStatement preparedStatement=connection.prepareStatement("SELECT * FROM VENDOR_PRODUCT WHERE PRODUCT_ID LIKE ? AND V_NO=?");
		logger.debug("Retrieving the details of a particular truck using truckId ");
		
		preparedStatement.setString(1,"%"+prodId+"%");
		preparedStatement.setString(2, vendorId);
		ResultSet resultSet=preparedStatement.executeQuery();
		ArrayList<VendorProduct> prodList = new ArrayList<VendorProduct>();
		while(resultSet.next())
		{
			String VNo = resultSet.getString("V_NO");
			String prodNo = resultSet.getString("PRODUCT_ID");
			int vat = resultSet.getInt("VAT");
			double service = resultSet.getDouble("SERVICE");
			VendorProduct product=new VendorProduct();
			product.setV_NO(VNo);
			product.setProducts(prodNo);
			product.setVat(vat);
			product.setServiceCharge(service);
