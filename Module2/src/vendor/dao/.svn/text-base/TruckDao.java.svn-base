...
public class TruckDao {
	...
	public String removeTruck(String tNo) throws SQLException {
		...
		PreparedStatement ps = connection
				.prepareStatement("DELETE FROM TRUCK WHERE T_NO NOT IN(SELECT T_NO FROM SHIPMENT)AND T_NO=?");
		ps.setString(1, tNo);
	...
	public ArrayList<Truck> searchTruck(String truckNo, String vendorId) throws SQLException {
		...
		PreparedStatement preparedStatement=connection.prepareStatement("SELECT * FROM TRUCK WHERE T_NO LIKE ? AND V_NO=?");
		logger.debug("Retrieving the details of a particular truck using truckId ");
		
		preparedStatement.setString(1,"%"+truckNo+"%");
		preparedStatement.setString(2, vendorId);
		ResultSet resultSet=preparedStatement.executeQuery();
		ArrayList<Truck> truckList = new ArrayList<Truck>();
		while(resultSet.next())
		{
			String truckNo1 = resultSet.getString("T_NO");
			String vendorNo = resultSet.getString("V_NO");
			int year = resultSet.getInt("YEAR");
			Truck truck = new Truck();
			truck.setTruckNo(truckNo1);
			truck.setOwner(vendorNo);
			truck.setYear(year);
			truckList.add(truck);
